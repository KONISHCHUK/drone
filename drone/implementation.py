...
ATM_WATCHDOG_URI = "https://atm:6064/watchdog"
...
REQUEST_TIMEOUT = 5  # Тайм-аут для запросов
...
class Drone:
    ...
    def __init__(self, coordinate, name, psswd):
        ...
        threading.Thread(target=self.self_diagnostic).start()
        threading.Thread(target=self.position_controller).start()
...
class Drone:
    ...
    def position_controller(self):
        ...
        if (abs(self.coordinate[0] - self.start_point[0]) < 1) and (
                abs(self.coordinate[1] - self.start_point[1]) < 1) and len(self.task_points) == 0:
            self.end_task()
    ...
    def end_task(self):
        try:
            ...
            logging.error(f'Error in end_task: {e}')
    ...
    def start(self, speed):
        ...
        while not len(self.task_points) == 0:
            ...
            if not self.emergency_stop.is_set() and self.motion_status == "Stopped":
                ...
                if abs(self.coordinate[0] - x) <= 1 and abs(self.coordinate[1] - y) <= 1 and abs(
                        self.coordinate[2] - z) <= 1:
    ...
    def send_position(self):
        ...
        try:
            ...
            logging.error(f'Error in send_position: {e}')
